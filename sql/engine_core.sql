...
select r.id,
       ...
       -- kept so downstream queries never have to join back to mf_holdings
       r.section,
       r.sub_section,
       asset_class(r.section, r.sub_section, r.is_security,
                   r.industry_rating, r.coupon_pct, r.isin, r.instrument_name) as asset_type,
       r.pct_to_nav,
       r.market_value_lacs,
