/* ============================================================================
   ...
   ----------------------------------------------------------------------------
   ₹ handling:  Rs. in all jsPDF-native text (cover/chrome/snapshot/infographics);
                ₹ preserved inside html2canvas-captured section bodies.
   ========================================================================== */
...
/** Short money string with Cr / L / K suffix, Rs. prefix (native-text safe). */
function fmtRs(n) {
  n = Math.round(Number(n) || 0);
  if (n >= 1e7) return 'Rs. ' + (n / 1e7).toFixed(2).replace(/\.?0+$/, '') + ' Cr';
  if (n >= 1e5) return 'Rs. ' + (n / 1e5).toFixed(2).replace(/\.?0+$/, '') + ' L';
...
function circularText(doc, text, cx, cy, radius, color, fontSize, startDeg = -90) {
  doc.setFont('helvetica', 'bold'); doc.setFontSize(fontSize); doc.setTextColor(...color);
...
function drawProgrammaticLogo(doc, x, y, width) {
  ...
  // tagline
  doc.setFont('helvetica', 'normal'); doc.setFontSize(4.6 * u); doc.setTextColor(...gold);
...
function drawFooter(doc, todayStr) {
  ...
  doc.text('NOT SEBI-REGISTERED INVESTMENT ADVICE — CONSULT A QUALIFIED ADVISOR BEFORE INVESTING',
    PW / 2, FOOTER_RULE_Y + 12, { align: 'center', charSpace: 1 });
}
...
function drawBodyBackdrop(doc) {
  ...
  withOpacity(doc, 0.10, () => {
    doc.setFont('helvetica', 'bold'); doc.setFontSize(6.5); doc.setTextColor(...IV.goldDeep);
...
function drawRiskMeter(doc, x, y, w, score, label) {
  doc.setFont('helvetica', 'bold'); doc.setFontSize(7.5); doc.setTextColor(...IV.goldDeep);
  ...
  // value pill
  doc.setFont('helvetica', 'bold'); doc.setFontSize(8); doc.setTextColor(...IV.navy);
...
function drawAllocationDonut(doc, cx, cy, rOut, segments, centreTop, centreBot) {
  ...
  doc.setFont('times', 'bold'); doc.setFontSize(12); doc.setTextColor(...IV.navy);
  ...
  segments.forEach(seg => {
    ...
    doc.setFont('helvetica', 'bold'); doc.setTextColor(...IV.text);
...
const na = Math.PI + p * Math.PI;
  ...
  // labels
  doc.setFont('helvetica', 'bold'); doc.setFontSize(7); doc.setTextColor(...IV.goldDeep);
  ...
  doc.setFont('courier', 'normal'); doc.setFontSize(6.6); doc.setTextColor(...IV.muted);
...
function drawProjectionCone(doc, x, y, w, h, years, series) {
  ...
  doc.setFont('courier', 'normal'); doc.setFontSize(6); doc.setTextColor(...IV.muted);
  ticks.forEach(t => {
    const tx = X(t);
    ...
    doc.text(fmtRs(series.mod[t]).replace('Rs. ', ''), tx, y + h + 7, { align: 'center' });
    doc.setTextColor(...IV.muted);
  });
  doc.setFont('helvetica', 'bold'); doc.setFontSize(7); doc.setTextColor(...IV.goldDeep);
...
function drawGlidePath(doc, x, y, w, years) {
  ...
  doc.setFont('helvetica', 'normal'); doc.setFontSize(6); doc.setTextColor(...IV.muted);
...
async function buildReportDoc() {
  ...
  const score = Number(p.riskScore) || 50;
  const rLabel = (typeof riskLabel === 'function') ? riskLabel(score, true) : 'Moderate';
  const horizon = parseInt(String(p.horizon || '15'), 10) || 15;
  const sipNum = Number(String(p.monthlySIP || '0').replace(/[^\d.]/g, '')) || 0;
  const sipStr = 'Rs. ' + (sipNum ? sipNum.toLocaleString('en-IN') : String(p.monthlySIP || '0'));
  ...
  const fv = (rate) => {
    const r = rate / 12, n = horizon * 12;
    return r ? sipNum * ((Math.pow(1 + r, n) - 1) / r) : sipNum * n;
  };
  const projMod = fv(0.11), projCons = fv(0.08), projOpt = fv(0.14);
  const target = projMod * 1.18;
const feasibility = (sipNum > 0 && target > 0) ? Math.min(118, (projMod / target) * 100) : 50;
  const coneSeries = { cons: [], mod: [], opt: [] };
  for (let y = 0; y <= horizon; y++) {
    const m = y * 12;
    const at = (rate) => { const r = rate / 12; return r ? sipNum * ((Math.pow(1 + r, m) - 1) / r) : sipNum * m; };
    coneSeries.cons.push(at(0.08)); coneSeries.mod.push(at(0.11)); coneSeries.opt.push(at(0.14));
  }

  const eq = Math.round(Math.min(80, Math.max(25, 25 + score * 0.55)));
  const intl = Math.round(eq * 0.18);
  const eqDom = eq - intl;
  const goldPct = 8, cashPct = 7;
  const debt = 100 - eqDom - intl - goldPct - cashPct;
  const allocation = [
    { label: 'Equity', v: eqDom, c: IV.navy },
    { label: 'Debt', v: debt, c: IV.gold },
    { label: 'Liquid', v: cashPct, c: IV.creamDeep },
    { label: 'Intl', v: intl, c: IV.sage },
    { label: 'Gold', v: goldPct, c: IV.bronze }
  ].filter(s => s.v > 0);

  /* ---- logo (graceful fallback) ----------------------------------------- */
  let logoImg = null;
  try { logoImg = await loadImageAsDataURL(IV_LOGO_URL); } catch (e) { logoImg = null; }
  ...
  function drawCoverPage() {
    ...
    doc.setFont('helvetica', 'bold'); doc.setFontSize(8); doc.setTextColor(...IV.goldDeep);
    ...
    rows.forEach((r, i) => {
      ...
      doc.setFont('helvetica', 'bold'); doc.setFontSize(10); doc.setTextColor(...IV.navy);
    ...
    doc.setFont('helvetica', 'bold'); doc.setFontSize(11); doc.setTextColor(...IV.navy);
    doc.text('CLARITY BEFORE YOU INVEST', PW / 2, gsY + 8, { align: 'center', charSpace: 2 });
    doc.setFont('helvetica', 'normal'); doc.setFontSize(7); doc.setTextColor(...IV.navyDark);
  ...
  function drawDashboardNative() {
    ...
    cards.forEach((c, i) => {
      ...
      doc.setFont('times', 'bold'); doc.setFontSize(17); doc.setTextColor(...IV.navy);
      doc.text(String(c.v), x + 13, y + 13, { baseline: 'middle' });
      doc.setFont('helvetica', 'bold'); doc.setFontSize(6.8); doc.setTextColor(...IV.goldDeep);
  ...
  async function captureEl(el, bg) {
    ...
    try {
      const canvas = await html2canvas(el, {
        scale: 2.4, useCORS: true, backgroundColor: bg, logging: false,
  ...
  function placeCapture(canvas, drawChrome) {
    ...
    while (sy < canvas.height - 1) {
      ...
      doc.addImage(tmp.toDataURL('image/png'), 'PNG', x, BODY_TOP, imgW, hpx / pxPerMm);
  ...
  function drawClosingPage() {
    ...
    doc.setFont('helvetica', 'bold'); doc.setFontSize(8); doc.setTextColor(...IV.gold);
    ...
    doc.setFont('helvetica', 'normal'); doc.setFontSize(6); doc.setTextColor(...IV.goldBright);
  ...
  try {
    ...
    if (chartsEl) {
      ...
      if (cCanvas) {
        ...
        doc.addImage(cCanvas.toDataURL('image/png'), 'PNG', MARGIN, BODY_TOP + 10,
          Math.min(imgW, pw), ph);
      }
    }
  ...
  toc.forEach(item => {
    ...
    doc.setFont('helvetica', 'bold'); doc.setFontSize(11); doc.setTextColor(...IV.gold);
    doc.text(item.num, MARGIN + 10, ty);
    doc.setFont('helvetica', 'normal'); doc.setFontSize(12); doc.setTextColor(...IV.navy);
